...
```text
Nicotine421Native.dll
Nicotine421Injector.exe
README.md
```
...
```powershell
.\Nicotine421Injector.exe <pid> .\Nicotine421Native.dll
```

The injector only performs `LoadLibraryW`. Once loaded, the DLL waits for the JVM and Minecraft `Client thread`, and loads the embedded JAR via its context ClassLoader. Fabric instances add the payload to the Knot ClassLoader via the Fabric Launcher API. Subsequently, the DLL registers nine native methods and calls `gg.nicotine.runtime.NativeBridge.start()`. Execution results are written to `Nicotine421-native.log` in the same directory as the DLL.
